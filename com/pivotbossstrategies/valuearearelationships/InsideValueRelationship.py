...
def getEquityData(equity_name) :
    cpr_day = date.today() - timedelta(days=10)
...
def printHigherValueDays(equity_name) :
    ...
    for row in data.itertuples():
        ...
        if (yesterday_day.tc < day_before_yesterday.tc and yesterday_day.tc > day_before_yesterday.bc) \
                and (yesterday_day.bc > day_before_yesterday.bc and yesterday_day.bc < day_before_yesterday.tc):
            if row.Open > (day_before_yesterday.tc) :
            ...
            if row.Open < (day_before_yesterday.bc) :
                #print(day_before_yesterday.cpr_date, yesterday_day.cpr_date)
