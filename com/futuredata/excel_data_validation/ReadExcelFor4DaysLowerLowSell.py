...
# During the 5th day if the value is 4 days low value - 1% then we will enter the trade
# Target = Trade_entry * 96%
# SL = Min(Trade_entry * 104%, Two days higher high + 1%)
...
def scan_candles_for_trade_info(four_day_low_val, row_index):
    ...
    trade_trigger_val = four_day_low_val * 0.99
    while True:
        if candle_data.low_val <= trade_trigger_val:
            print('Trade Triggered on ', candle_data.date, ' value :', round(trade_trigger_val, 2))
            trade_stats.increase_trade_hit_count()
            trade_info.set_values(True, trade_trigger_val, round(trade_trigger_val * 0.96, 2),
                                  round(trade_trigger_val * 1.04, 2))
...
def get_two_day_high_val(third_day_high_val, fourth_day_high_val):
    if third_day_high_val > fourth_day_high_val:
        return third_day_high_val
    return fourth_day_high_val


def get_sl_val(trade_info_sl_level):
    low_val = round(two_day_high_val * 1.01)
...
for fut_key in future_dict:
    ...
    while fifth_day_data is not None:
        ...
        two_day_high_val = get_two_day_high_val(third_day_data.high_val, fourth_day_data.high_val)
        if trade_info.is_trade_triggered:
            scan_candles_for_target_or_sl_hit(fourth_day_data.row_index)
        else:
            four_day_low_value = get_low_value(first_day_data.low_val, second_day_data.low_val,
                                                third_day_data.low_val, fourth_day_data.low_val)
            if fifth_day_data.low_val <= four_day_low_value * 0.99:
                scan_candles_for_trade_info(four_day_low_value, fourth_day_data.row_index)
