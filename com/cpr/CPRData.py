...
def getEquityData(equity_name, startDate, endDate) :
 data = get_history(symbol=equity_name, start=startDate, end=endDate)
 return data
...
def printExtremeRangeDays(equityName) :
 ...
 for row in data.itertuples():
  ...
  if day_low != 0 and day_high != 0:
   ...
   if (row.Open > S1 or row.Open > R1) and (day_low >= row.Open or day_high <= row.Open):
