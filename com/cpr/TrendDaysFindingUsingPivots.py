# This logic displays the product name & it's trending days information.
# If the equity price starts above the R1 or below S1 then it prints that information.

from datetime import date, timedelta
from nsepy import get_history

from cpr import CPRData
...
nse_smallcap_250 = ['THYROCARE', 'MCX', 'BSOFT', 'WESTLIFE','BHARATRAS', 'ADVENZYMES', 'MOIL', 'BIRLACORPN',
                      ...
                      'SWANENERGY', 'HFCL', 'HATHWAY', 'HERITGFOOD', 'ASHOKA', 'MAHSEAMLES']
...
for equity_name in nseShareNames:
    ...
    for row in data.itertuples() :
        ...
        if row.Open > today_cpr_data.R1:
            print(equity_name, 'uptrend on', row.Index)

        if row.Open < today_cpr_data.S1:
            print(equity_name, 'downtrend on', row.Index)
