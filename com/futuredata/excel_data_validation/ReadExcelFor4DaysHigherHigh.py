...
# During the 5th day if the value is 4 days high value + 1% then we will enter the trade
# Target = Trade_entry * 104%
# SL = Max(Trade_entry * 96%, Two days lower low - 1%)
...
def get_four_day_high_value(first_day_high_val, second_day_high_val,
                            third_day_high_val, fourth_day_high_val):
    high_val = first_day_high_val
    if second_day_high_val > high_val:
        high_val = second_day_high_val
    if third_day_high_val > high_val:
        high_val = third_day_high_val
    if fourth_day_high_val > high_val:
        high_val = fourth_day_high_val
    return high_val


def get_two_day_low_val(third_day_low_val, fourth_day_low_val):
    if third_day_low_val < fourth_day_low_val:
        return third_day_low_val
    return fourth_day_low_val


def scan_candles_for_trade_info(four_day_high_val, row_index):
    candle_data = get_candle_data(row_index)  # First Row
    trade_trigger_val = four_day_high_val * 1.01
    while True:
        if candle_data.high_val >= trade_trigger_val:
            print('Trade Triggered on ', candle_data.date, ' Val: ', round(trade_trigger_val, 2))
            trade_stats.increase_trade_hit_count()
            trade_info.set_values(True, trade_trigger_val, round(trade_trigger_val * 1.04, 2),
                                  round(trade_trigger_val * 0.96, 2))
...
def scan_candles_for_target_or_sl_hit(row_index):
    candle_data = get_candle_data(row_index)  # First Row
    sl_val = get_sl_val(trade_info.trade_sl_level)
    while True:
        if candle_data.low_val <= sl_val:
            print('SL hit on ', candle_data.date, ' value = ', sl_val)
...
def get_sl_val(trade_info_sl_level):
    low_val = round(two_day_low_val * 0.99)
    if trade_info_sl_level < low_val:
        return low_val
    return trade_info_sl_level
...
for fut_key in future_dict:
    ...
    future_name = future_dict.get(fut_key)
    ...
    while fifth_day_data is not None:
        two_day_low_val = get_two_day_low_val(third_day_data.low_val, fourth_day_data.low_val)
        if trade_info.is_trade_triggered:
            scan_candles_for_target_or_sl_hit(fourth_day_data.row_index)
        else:
            four_day_high_value = get_four_day_high_value(first_day_data.high_val, second_day_data.high_val,
                                                        third_day_data.high_val, fourth_day_data.high_val)
            if fifth_day_data.high_val >= four_day_high_value * 1.01:
                scan_candles_for_trade_info(four_day_high_value, fourth_day_data.row_index)
