...
#   This code back tests the data for November data
...
def get_five_day_low_high_values(start_date, nov_data):
    low_vals = dict(nov_data['Low'])
...
def get_trade_stats_on_futures():
    ...
    for future_name in nse_future_names:
        start_date = datetime.datetime(2020, 11, 6).date()

        #if future_name == 'L%26TFH' or future_name == 'M%26M' or future_name == 'M%26MFIN':
        #    continue
        nov_data = get_nov_future_data(future_name)
        low_vals = dict(nov_data['Low'])
        if low_vals == {}:
            continue
        while start_date <= datetime.datetime(2020, 11, 26).date():
            current_day_low_val = low_vals[start_date]

            day_data = get_five_day_low_high_values(start_date, nov_data)
            ...
            if current_day_low_val <= trade_entry_val:
                ...
                target_hit_date = get_target_hit_date(start_date, nov_data, day_data.high_val)
                ...
                if target_hit_date is None:
                    ...
                    sl_hit_date = get_sl_hit_date(start_date, nov_data, sl_hit_val)
...
def get_target_hit_date(start_date, nov_data, target_val):
    high_vals = dict(nov_data['High'])
...
def get_sl_hit_date(start_date, nov_data, sl_value):
    high_vals = dict(nov_data['Low'])
    while start_date < date.today():
...
print('Neither target hit nor SL hit count = ', stats.no_result)
