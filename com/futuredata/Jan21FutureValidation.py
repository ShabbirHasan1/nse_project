...
#   This code back tests the data from  Jan 1st onwards
...
def get_five_day_low_high_values(start_date, jan_month_data):
    low_vals = dict(jan_month_data['Low'])
    ...
    high_vals = dict(jan_month_data['High'])
...
def get_trade_stats_on_futures():
    ...
    for future_name in nse_future_names:
        start_date = datetime.datetime(2021, 1, 7).date()
        #if future_name == 'L%26TFH' or future_name == 'M%26M' or future_name == 'M%26MFIN':
        #    continue
        jan_month_data = get_jan_month_future_data(future_name)
        low_vals = dict(jan_month_data['Low'])
        if low_vals == {}:
            continue
        while start_date <= date.today():
            current_day_low_val = low_vals[start_date]
            day_data = get_five_day_low_high_values(start_date, jan_month_data)
            ...
            if current_day_low_val <= trade_entry_val:
                ...
                target_hit_date = get_target_hit_date(start_date, jan_month_data, day_data.high_val)
                ...
                if target_hit_date is None:
                    ...
                    sl_hit_date = get_sl_hit_date(start_date, jan_month_data, sl_hit_val)
                    ...
                    #start_date = get_date(-1, target_hit_date, low_vals.keys())     # for next day
...
def get_target_hit_date(start_date, jan_month_data, target_val):
    high_vals = dict(jan_month_data['High'])
    while start_date < date.today():
...
def get_sl_hit_date(start_date, jan_month_data, sl_value):
    high_vals = dict(jan_month_data['Low'])
    while start_date < date.today():
